...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint8 private _hr_percentage = 1;
    uint8 private _marketing_percentage = 1;
    uint8 private _reward_percentage = 2;
    uint8 private _transaction_burn = 4;
    address private _hr_address;
     ...
     * construction.
     */
    constructor (string memory name_, string memory symbol_, uint8 decimal_, uint256 totalSupply_, address hr_address_, address marketing_address_) {
        _name = name_;
        _symbol = symbol_;
        _decimal = decimal_;
        _hr_address = hr_address_;
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        if(account == _hr_address || account == _marketing_address) return _balances[account];
    ...
    function _transferStandard(address sender_address, address recipient_address, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 hr_amount, uint256 marketing_amount, uint256 burn_amount) = _getTValues(tAmount);
        uint256 currentRate =  _getRate();
        ...
        uint256 sub_amount = rAmount - hr_amount - marketing_amount - burn_amount;
        _rOwned[sender] = _rOwned[sender].sub(sub_amount);
        
        
        // update amount in recipient address
        uint256 transfer_amount = rTransferAmount - hr_amount - marketing_amount - burn_amount;
        _rOwned[recipient] = _rOwned[recipient].add(transfer_amount);
        
        // reflect fee
        _reflectFee(rFee, tFee);
        
        // add amount in hr and marketing address
        _balances[_hr_address] += hr_amount;
        _balances[_marketing_address] += marketing_amount;
        
        emit Transfer(sender, recipient, tTransferAmount);
        emit Transfer(sender, _hr_address, hr_amount);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 hr_amount = totalAmount.mul(_hr_percentage).div(10**3);
        ...
        uint256 tTransferAmount = totalAmount.sub(tFee).sub(hr_amount).sub(marketing_amount).sub(burn_amount);
        return (tTransferAmount, tFee, hr_amount, marketing_amount, burn_amount);
    }
